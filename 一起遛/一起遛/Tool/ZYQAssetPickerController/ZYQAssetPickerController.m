...
+ (void)initialize
{
    checkedIcon     = [UIImage imageNamed:@"photoSelect"];
    uncheckedIcon   = [UIImage imageNamed:@"uncheckPhoto"];
...
-(id)initWithFrame:(CGRect)frame{
    if (self=[super initWithFrame:frame]) {
        ...
        _selectView.image = uncheckedIcon;
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _tapAssetView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
- (void)bind:(NSArray *)assets selectionFilter:(NSPredicate*)selectionFilter minimumInteritemSpacing:(float)minimumInteritemSpacing minimumLineSpacing:(float)minimumLineSpacing columns:(int)columns assetViewX:(float)assetViewX{
    ...
    assetCelLength = (kPopoverContentSize.width - (columns - 1) * minimumInteritemSpacing) / columns;
    
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-1)) {
                ZYQAssetView *assetView=[[ZYQAssetView alloc] initWithFrame:CGRectMake(assetViewX+(assetCelLength+minimumInteritemSpacing)*i, minimumLineSpacing-1, assetCelLength, assetCelLength)];
                ...
                ((ZYQAssetView*)self.contentView.subviews[i]).frame=CGRectMake(assetViewX+(assetCelLength+minimumInteritemSpacing)*(i), minimumLineSpacing-1, assetCelLength, assetCelLength);
        ...
        for (NSInteger i=self.contentView.subviews.count; i>0; i--) {
            if (i>assets.count) {
                ...
                ((ZYQAssetView*)self.contentView.subviews[i-1]).frame=CGRectMake(assetViewX+(assetCelLength+minimumInteritemSpacing)*(i-1), minimumLineSpacing-1, assetCelLength, assetCelLength);
...
- (void)setupButtons
{
    ...
    [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"confirm_n"]
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return (tableView.frame.size.width - columns * minimumInteritemSpacing) / 3 + minimumLineSpacing;
}
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset selected:(BOOL)selected
{
    ...
    if (!selected && (_indexPathsForSelectedItems.count >= vc.maximumNumberOfSelection)) {
        ...
        [MBProgressHUD ShowTips:msg delayTime:2.0 atView:nil];
    }
...
- (void)localize
{
    self.title = NSLocalizedString(@"全部图片", nil);
    self.navigationController.navigationBar.barTintColor = [UIColor mainColor];
